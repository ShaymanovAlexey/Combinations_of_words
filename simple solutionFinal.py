...
for key in word_stat_dict:
    ...
    #most_freq_dict_list[key] = [x for x in word_stat_dict[key].keys() if word_stat_dict[key][x] > ]
...
for line in fl:
    ...
    if key in most_freq_dict:
        ...
        if p1.tag.POS =='VERB' and p2.tag.POS =='ADJF':
            ...
            if p2 != None:
                word_good = p2.word
        elif p1.tag.POS =='NOUN' and p2.tag.POS =='VERB':
            num_p1 = p1.tag.number   
            p2 = p2.inflect({num_p1})
            if p2 != None:
                word_good = p2.word
        ...
        #print(word_good, " bum bum ", sum_d)
        out_fl.write('%s,%s %s\n' % (Id, word1, word_good))
    else:
        count +=1
        if key_one in most_freq_dict_single.keys():
